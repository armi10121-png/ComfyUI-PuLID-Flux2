"""
train_pulid.py - VERSION OPTIMISÉE COMPLÈTE
============================================
...
class TrainingConfig:
    ...
    FAKE_IMG_TOKENS_COUNT = 64  # Flux.2 travaille avec ~64x64 patches
...
class FaceDataset(Dataset):
    ...
    def _augment(self, img: Image.Image) -> Image.Image:
        """
        Augmentation adaptée aux visages pour améliorer la généralisation.
        - Pas de flip horizontal (inverse gauche/droite du visage)
        - Rotation légère ±15° (augmenté de ±10° pour plus de diversité)
        - Brightness / contrast / saturation modérés
        """
        # Rotation légère (augmentée pour plus de robustesse)
        ...
        # Brightness
        if random.random() > 0.5:
            factor = random.uniform(0.8, 1.2)  # Plage élargie
        ...
        # Légère variation de netteté (nouveau)
...
def load_flux2_klein(model_path: str, device: torch.device):
    ...
    try:
        ...
        # Tenter de charger via diffusers
        if os.path.isdir(model_path):
            ...
            # Si c'est un .safetensors, essayer via from_single_file
...
def load_pulid_module(comfyui_path: str, dim: int, device: torch.device):
    ...
    if not pulid_node_path:
        raise FileNotFoundError(
            ...
            "  git clone https://github.com/YOUR_REPO/ComfyUI-PuLID-Flux2"
        )
...
class PuLIDKleinTrainer:
    ...
    Phase 2 : IDFormer + PerceiverCA (avec Flux gelé)
        ...
        - Loss : identity + perceiver coherence + regularization
    """
    ...
    def __init__(self, args):
        ...
        self.dataloader = DataLoader(
            ...
            pin_memory=True,
            drop_last=True,
            persistent_workers=True  # Accélère les itérations
        )
        
        # ── Modèles ───────────────────────────────────────────────────────────
        log.info("Chargement des modèles...")
        
        # EVA-CLIP (toujours nécessaire)
        ...
        if args.resume:
            ...
            if missing:
                log.warning(f"⚠️  Clés manquantes : {missing[:5]}...")
            if unexpected:
                log.warning(f"⚠️  Clés inattendues : {unexpected[:5]}...")
        ...
        self.scheduler = CosineAnnealingLR(
            self.optimizer,
            T_max=total_steps,
            eta_min=args.lr / 100  # LR min = 1% de LR initial
        )
    ...
    def encode_eva_clip(self, images: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            # Forward
            features = self.eva_clip(x)  # [B, 768]
    ...
    def train_step_phase1(self, images: torch.Tensor, id_embeds: torch.Tensor):
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            # Cosine distance loss (1 - cosine_sim)
    ...
    def train_step_phase2(self, images: torch.Tensor, id_embeds: torch.Tensor):
        ...
        Loss :
            - identity : maintient la qualité des tokens d'identité
            - perceiver : cohérence des corrections cross-attention
            - reg : régularisation L2
        """
        images = images.to(self.device, dtype=torch.bfloat16)
        id_embeds = id_embeds.to(self.device, dtype=torch.bfloat16)
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            # 1. EVA-CLIP encoding
            clip_embed = self.encode_eva_clip(images)
            ...
            # 4. Loss PerceiverCA : apprendre à injecter dans les image tokens
            loss_perceiver = torch.tensor(
                0.0,
                device=self.device,
            ...
            if self.flux is not None:
                ...
                # ✅ FIX CRITIQUE : Créer de VRAIS tokens synthétiques
                # Ils représentent des image tokens (pas les id_tokens !)
                fake_img_tokens = torch.randn(
                    B,
                    self.config.FAKE_IMG_TOKENS_COUNT,  # 64 tokens
                    dim,
                    device=self.device,
                    dtype=torch.bfloat16
                )
                # Normaliser avec variance réaliste
                fake_img_tokens = F.normalize(fake_img_tokens, dim=-1) * 2.0
                
                embed = id_tokens.to(dtype=torch.bfloat16)
                ca_corrections = []
                
                # Appliquer les PerceiverCA
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Nombre d'epochs (auto si non spécifié : 25 pour Phase 1, 15 pour Phase 2)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Batch size (auto si non spécifié : 4 pour Phase 1, 2 pour Phase 2)"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Learning rate (auto si non spécifié : 1e-4 pour Phase 1, 5e-5 pour Phase 2)"
    )
    ...
    if args.epochs is None:
        args.epochs = (
            config.PHASE1_EPOCHS if args.phase == 1
            else config.PHASE2_EPOCHS
        )
    
    if args.batch_size is None:
        args.batch_size = (
            config.PHASE1_BATCH_SIZE if args.phase == 1
            else config.PHASE2_BATCH_SIZE
        )
    
    if args.lr is None:
        args.lr = (
            config.PHASE1_LR if args.phase == 1
            else config.PHASE2_LR
        )
